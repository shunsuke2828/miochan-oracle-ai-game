...
ターン送信時は自然言語回答を先にADBへ保存して即座に次の問いを返します。`UTL_TO_EMBEDDING`による1536次元化はHTTPレスポンス後のバックグラウンド処理で行い、画面は状態APIから暫定スコアを反映します。タイムアップまたは早期クリア時には未処理回答を回収し、Geminiによる全ターン一括品質評価とMIO-RSの順序再計算を済ませてから最終結果を返します。
...
- `POST /api/mio/sessions/{session_id}/finish` — タイムアップ確定
